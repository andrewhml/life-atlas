...
**Status:** Complete (closed 2026-05-26). Phase 1 audit captured; Phase 2 end-to-end on AM2 + AM5 (`/opt/homebrew/bin/betterdisplaycli` from the Homebrew formula on both, Atlas-symlinked `.ini`, services running cleanly with verified `betterdisplaycli` invocations). Phase 3 abandoned during audit. Two minor follow-ups deferred (non-blocking, recorded in status log + step 8/10 of Phase 2): AM5 USB-disconnect path verification on next unplug, optional removal of AM2's legacy `/usr/local/bin/betterdisplaycli`.
...
- 2026-05-26 — **Plan closed (Status: Complete).** Both machines converged on the Homebrew-formula `betterdisplaycli` (Brewfile-managed) reading the Atlas-symlinked `.ini`. Outstanding items are non-blocking and tracked in step 8 (passive USB-disconnect verification) and step 10 (optional AM2 cleanup); no further coordinated work required.
