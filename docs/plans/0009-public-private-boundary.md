...
- 2026-05-27 — **Phase 2 complete.** (Step 1) "Reference implementation" tables extracted from `CLAUDE.md` and `README.md` → merged into `~/Atlas/config/atlas/reference-implementation.md`; pointers left in both files. Sanitized incidental leaks discovered along the way: employer-name references in CLAUDE.md tables, "UGREEN"/"AM5" in the language-rule examples, "Syntheus" / employer references in `folder-schemas/workspace.md`. (Steps 2–4) Subagent merged the 8 `device-schemas/*.md` files + the existing `device-schemas/inventory.yaml` into a single enriched `~/Atlas/docs/gear/inventory.yaml` (11 device records, all with `notes:`, 1449 lines / 61 KB, `yq` parses). Migration manifest at `~/Atlas/docs/gear/.migration-manifest-0009.md` records source→destination mapping; diff-check shows zero missing paragraphs. Schema bumped 1 → 2; `macbook-work` old `notes:` one-liner renamed to `summary:`. (Step 5) Deleted the 8 device `.md` files + old `device-schemas/inventory.yaml` via `git rm`; nulled vestigial `schema_doc:` / `structure_doc:` references in the private yaml. (Steps 6–7) Created `device-schemas/inventory.template.yaml` (skeleton with placeholder values, parses) + rewrote `device-schemas/README.md` (one-yaml design, where filled-in lives, conventions, AI-context skill pointer). (Step 8) Copied plans 0001–0008 to `~/Atlas/workspace/atlas-ops/plans/`; `git rm`'d from public repo (Plan 0009 + `docs/plans/README.md` retained). (Step 9) Rewrote `docs/plans/README.md` to scope public `docs/plans/` to meta-plans about the public guide; private operational plans live in `atlas-ops/plans/`. (Step 10) Fixed 7 broken cross-references in `environment/nas-setup.md`, `environment/pc_defaults.md`, `folder-schemas/media.md`, `environment/apps-manual.md` (×2), `tools/photo-coverage/README.md`, `environment/claude-setup.md` (×3) — pointed at the new inventory.yaml or genericized.
- 2026-05-27 — **Phase 3 complete.** (Step 1) Brewfile sanitized: header comment dropped the AM2 codename + plan 0007 phase ref; the WeasyPrint block no longer names the employer project (now reads as "PDF pipelines that need WeasyPrint native libs at /opt/homebrew/lib"); residual `(plan 0007 Phase 2f)`, `(plans 0002, 0005)`, `(plan 0008)`, `(plan 0003)` annotations stripped from the gitleaks/immich/display-switch/p10k/ghostty lines; "AM5 onboarding" date-stamp comment in the App Store apps block genericized. (Step 2) Added `brew "git-filter-repo"` under CLI: security / secrets, alongside `gitleaks`. (Step 3) `apps-manual.md` sanitized: AM2 → "source workstation" / AM5 → "new workstation"; dropped plan 0007 phase 4/5/5f refs, the Carbonizer/Carbonizer_BU/Noisy Cricket backup-task enumeration (now points at the inventory.yaml external-storage records), the "brother's UGREEN" family reference, the UGREEN DXP6800/Peddocks2 pair (now a generic "Vendor NAS app (if applicable)" section), specific extension counts and font family enumerations, the Keynote-on-AM5 note, and remaining AM2/AM5 tokens in the LaunchAgents and system-extensions tables. (Step 4) Walked the rest of `environment/`: `nas-setup.md` dropped `Peddocks2` from title + intro and genericized the vendor-app TODO line; `macos_environment_init.sh` replaced the `Syntheus` example folder + alias with `Consulting/` (broader rewrite still tracked as drift #1); `claude-setup.md` heavily rewritten — the personal-repo `andrewhml/claude-harness` is now `<your-github>/claude-harness`, AM2/AM5 became "first machine"/"new device", plan 0007 Phase 2a–2k breadcrumbs collapsed into a numbered summary without phase refs, `/Users/andrewlee/.claude/...` typo replaced with `${HOME}/.claude/...`, hardcoded `~/workspace/personal/life-atlas/...` invocations replaced with `<life-atlas>` placeholder + restoration instructions, and the personal AM2-cutover paragraph deleted. Other `environment/` files (`brewfile.md`, `shell-setup.md`, `pc_defaults.md`, `macos_defaults.md`, `keyboard_shortcuts.md`, `setup-claude-secrets.sh`, `sync-shell-config.sh`, `shell-aliases.sh`, `devtools.sh`, `defaults_settings.sh`, `macos_create_photo_year.sh`) audited — already clean of deny-list tokens. Final grep across `environment/` for AM2/AM5/Peddocks/Carbonizer/Noisy Cricket/Syntheus/andrewhml-harness/andrewlee/andrew.hml/plan 0007/`/Users/andrewhml`/email returns zero hits.
