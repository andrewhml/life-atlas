...
**Steps:**
1. Brewfile: scrub WHY comments that reference employer projects (the WeasyPrint block currently names the user's employer project; rewrite as "for PDF pipelines that need WeasyPrint native libs at `/opt/homebrew/lib`")
...
2. **Sanitize this plan file in-place.** Replace every specific token enumeration with categorical descriptions tied to the categories tracked in the private deny-list at `~/Atlas/config/atlas/lint-denylist.txt` — device codenames become "device codenames"; specific user email becomes "the user's email"; user-home-path literals become "the user-home path"; employer name becomes "the user's employer name". Goal: the plan remains a useful HOW record for anyone reading the public guide, without revealing the user's specific WHAT. After this step, the lint passes against this file too — the Phase 2/4 temporary exception for `0009-public-private-boundary.md` closes.
...
- 2026-05-27 — **Phase 0 complete.** Tool preflight: `gitleaks 8.30.1`, `yq 4.53.2` present; `git-filter-repo` deferred (only needed on findings). `git fetch --all --prune --tags` ran; 8 refs recorded in `/tmp/0009-refs-scanned.txt` (3 local branches + 4 remote refs + HEAD). `gitleaks detect --source . --log-opts="--all"` scanned 40 commits / 595 KB / 70 ms: **no leaks found**. No credential rotation or history rewrite required. PII-class exposure remains accepted per Threat Model.
- 2026-05-27 — **Phase 1 complete.** Preflight: `gear` exists lowercase in `~/Atlas/docs/`; no duplicate `Gear` — clean. (Side observation: `Travel/` is the only remaining uppercase sibling among the 13 docs/ categories — user-managed outside this plan.) Created `~/Atlas/config/atlas/` and `~/Atlas/workspace/atlas-ops/plans/`; `~/Atlas/docs/gear/` already existed from the user's manual rename. `.claude/settings.json` expanded with the 8 new write rules (5 repo-side, 3 Atlas-side narrow subtrees) — used `//absolute/path/*` syntax for Atlas, no blanket `~/Atlas/config/*` allow so `keys/` segregation stays structural. CLAUDE.md "Permissions & constraints" and "Atlas read/write boundaries" sections rewritten to match the new settings; added "Pattern→reference bindings" and "Hook install on every clone" subsections. `docs/` category list lowercased to reflect the user's manual rename.
- 2026-05-27 — **Phase 2 complete.** (Step 1) "Reference implementation" tables extracted from `CLAUDE.md` and `README.md` → merged into `~/Atlas/config/atlas/reference-implementation.md`; pointers left in both files. Sanitized incidental leaks discovered along the way: employer-name references in CLAUDE.md tables, vendor- and device-codename mentions in the language-rule examples, employer-name references in `folder-schemas/workspace.md`. (Steps 2–4) Subagent merged the 8 `device-schemas/*.md` files + the existing `device-schemas/inventory.yaml` into a single enriched `~/Atlas/docs/gear/inventory.yaml` (11 device records, all with `notes:`, 1449 lines / 61 KB, `yq` parses). Migration manifest at `~/Atlas/docs/gear/.migration-manifest-0009.md` records source→destination mapping; diff-check shows zero missing paragraphs. Schema bumped 1 → 2; one device's old `notes:` one-liner renamed to `summary:` to free the `notes:` field. (Step 5) Deleted the 8 device `.md` files + old `device-schemas/inventory.yaml` via `git rm`; nulled vestigial `schema_doc:` / `structure_doc:` references in the private yaml. (Steps 6–7) Created `device-schemas/inventory.template.yaml` (skeleton with placeholder values, parses) + rewrote `device-schemas/README.md` (one-yaml design, where filled-in lives, conventions, AI-context skill pointer). (Step 8) Copied plans 0001–0008 to `~/Atlas/workspace/atlas-ops/plans/`; `git rm`'d from public repo (Plan 0009 + `docs/plans/README.md` retained). (Step 9) Rewrote `docs/plans/README.md` to scope public `docs/plans/` to meta-plans about the public guide; private operational plans live in `atlas-ops/plans/`. (Step 10) Fixed 7 broken cross-references in `environment/nas-setup.md`, `environment/pc_defaults.md`, `folder-schemas/media.md`, `environment/apps-manual.md` (×2), `tools/photo-coverage/README.md`, `environment/claude-setup.md` (×3) — pointed at the new inventory.yaml or genericized.
- 2026-05-27 — **Phase 3 complete.** (Step 1) Brewfile sanitized: header comment dropped a device codename + a plan 0007 phase ref; the WeasyPrint block no longer names the employer project (now reads as "PDF pipelines that need WeasyPrint native libs at /opt/homebrew/lib"); residual plan 0002/0003/0005/0007/0008 phase annotations stripped from the gitleaks/immich/display-switch/p10k/ghostty lines; new-workstation-onboarding date-stamp comment in the App Store apps block genericized. (Step 2) Added `brew "git-filter-repo"` under CLI: security / secrets, alongside `gitleaks`. (Step 3) `apps-manual.md` sanitized: source/new workstation codenames replaced with "source workstation"/"new workstation"; dropped plan 0007 phase refs, the external-SSD backup-task enumeration that named specific volume codenames (now points at the inventory.yaml external-storage records), a family-NAS reference, the vendor-NAS model+codename pair (now a generic "Vendor NAS app (if applicable)" section), specific browser-extension counts and font-family enumerations, a workstation-specific Keynote note, and remaining workstation-codename tokens in the LaunchAgents and system-extensions tables. (Step 4) Walked the rest of `environment/`: `nas-setup.md` dropped the NAS codename from title + intro and genericized the vendor-app TODO line; `macos_environment_init.sh` replaced the employer-named example folder + alias with `Consulting/` (broader rewrite still tracked as drift #1); `claude-setup.md` heavily rewritten — the private claude-harness repo URL is now a `<your-github>/claude-harness` placeholder, workstation codenames became "first machine"/"new device", plan 0007 Phase 2a–2k breadcrumbs collapsed into a numbered summary without phase refs, a stale username typo in a JSON example replaced with `${HOME}/.claude/...`, hardcoded life-atlas-clone paths replaced with a `<life-atlas>` placeholder + restoration instructions, and a personal cutover paragraph deleted. Other `environment/` files (`brewfile.md`, `shell-setup.md`, `pc_defaults.md`, `macos_defaults.md`, `keyboard_shortcuts.md`, `setup-claude-secrets.sh`, `sync-shell-config.sh`, `shell-aliases.sh`, `devtools.sh`, `defaults_settings.sh`, `macos_create_photo_year.sh`) audited — already clean of deny-list tokens. Final grep across `environment/` for every deny-list category (device codenames, employer name, private-repo name, user-home path, user email) returns zero hits.
- 2026-05-27 — **Phase 4 complete.** Shipped the lint infrastructure: `scripts/lint-no-personal-data.sh` (engine only, no personal tokens) reads regex deny-list lines from `${LINT_DENYLIST:-$HOME/Atlas/config/atlas/lint-denylist.txt}` and passes them raw to `grep -E`. Two modes — working-tree (lint every tracked file) and `--staged` (reads the staged blob via `git show :file`, closing the partial-stage bypass where a clean working tree could hide a tainted staged version). `scripts/lint-denylist.example.txt` ships as the seed template, grouped by token shape with comment headers explaining when `\b` applies and when it silently fails. `scripts/install-hooks.sh` writes an idempotent shim at `.git/hooks/pre-commit` that invokes the engine in `--staged` mode; warns (not fails) if the private deny-list isn't present yet. Created the real `~/Atlas/config/atlas/lint-denylist.txt` with one regex per category tracked by the threat model. Fixed a Phase 1 oversight the lint correctly caught: `.claude/settings.json`'s three Atlas write rules used literal user-home-path strings (a real leak at HEAD) — replaced with `${HOME}/Atlas/...`; runtime-tested via a throwaway Write that Claude's permission classifier expands the variable. Tests: working-tree lint hit only on this plan file (the expected exception, closed by Phase 5 step 2); a word-like deny-list-token commit was blocked; a path-like deny-list-token commit was blocked (the critical \b-on-path failure-mode guard); a clean commit passed.
- 2026-05-27 — **Phase 5 complete.** (Step 1) Manual audit replay (the slash command isn't agent-invokable): one new audit finding — `README.md` repo-layout still listed the deleted device-schema `.md` files from Phase 2; fixed in this commit and added `scripts/` to both `README.md` and `CLAUDE.md` repo-layout sections (also missing). No broken markdown cross-refs introduced by the migrations; the `Kit/` mentions are intentional (CLAUDE.md cautionary note + audit/sync-check skill specs). One pre-existing inconsistency surfaced (`folder-schemas/config.md` describes a symlink-and-snapshot sync model for `~/.claude/` that contradicts the harness-git-repo model in `environment/claude-setup.md`); captured in session-scratch for a separate drift issue — pre-dates plan 0009. (Step 2) Sanitized this plan file in-place: replaced every specific token enumeration with categorical descriptions (device codenames, user email, user-home path, employer name, private-repo URL). (Step 3) Full-repo working-tree lint now passes with zero hits across all 48 tracked files. (Step 5) Status flipped to Complete. (Step 6) Drift issues closed as moot: #34 (`device-schemas/macbook-personal.md` reconcile — the file no longer exists, replaced by `~/Atlas/docs/gear/inventory.yaml`'s device record) and #14 (NAS-codename naming drift — no codename references remain in the public repo).
